...
AotHarness::AotHarness(Core::System& system, DispatchFunc dispatch, DispatchFunc interp_dispatch)
    : m_system(system), m_ppc_state(system.GetPPCState()), m_dispatch(dispatch),
      m_interp_dispatch(interp_dispatch)
{
}

AotHarness::~AotHarness()
{
  std::free(m_ram_shadow);
  std::free(m_ram_shadow_aot);
}

void AotHarness::RequestShutdown()
{
  s_shutdown_requested.store(true);
}

std::unique_ptr<AotHarness> AotHarness::MaybeCreate(Core::System& system,
                                                    const std::optional<AotGameEntry>& entry,
                                                    DispatchFunc dispatch,
                                                    DispatchFunc interp_dispatch)
{
  // Fallback tracking works with the production fast loop; enable as a side
  // effect without forcing the instrumented loop.
  if (std::getenv("AOT_TRACK_FALLBACKS"))
    aot_enable_fallback_tracking();

  const bool diff_mode = Config::Get(Config::MAIN_DEBUG_AOT_DIFF_MODE);
  const bool engaged = diff_mode || std::getenv("AOT_COMPARE") || std::getenv("AOT_INTERP_ONLY") ||
                       std::getenv("AOT_SWITCH_AT") || std::getenv("AOT_LOG_PC") ||
                       std::getenv("AOT_DUMP_FRAME") || std::getenv("AOT_LOAD_STATE");
  if (!engaged)
    return nullptr;

  auto harness = std::make_unique<AotHarness>(system, dispatch, interp_dispatch);
  ...
  if (entry)
  {
    ...
    for (u32 i = 0; i < entry->module_block_size_count; i++)
    {
      ...
      harness->m_module_block_sizes[(static_cast<u64>(mb.module_id) << 32) |
                                    (static_cast<u64>(mb.section) << 24) | mb.offset] =
          mb.num_instructions;
    }
    INFO_LOG_FMT(AOT, "AotHarness: {} block boundaries (+{} module blocks) from the registry",
                 harness->m_block_sizes.size(), harness->m_module_block_sizes.size());
  }
  ...
  if ((diff_mode || std::getenv("AOT_COMPARE")) && harness->m_block_sizes.empty())
  {
    ...
    fmt::print(stderr, "AOT_COMPARE: NO BLOCK BOUNDARIES — nothing will be compared. "
                       "Rebuild the AOT library with AOT_HARNESS=1.\n");
  }
...
void AotHarness::OnNewField(Core::System& system)
{
        ...
        else if (aot_fn && block_size > 0)
        {
          ...
          if (!regs_match || !ram_match)
          {
            ...
            fmt::print(stderr, "\n=== AOT_COMPARE: First divergence at block {:#010x} "
                       ...
                       block_pc, compare_count, regs_match ? "match" : "DIFFER",
                       !full_ram ? "unchecked" : (ram_match ? "match" : "DIFFER"));
            LogDivergence(block_pc, num_instr, pre, aot_result, interp_result, stderr);
            // BlockAccessesMMIO only sees pre-state GPRs and lis patterns; a
            // pointer LOADED inside the block (e.g. an AI/DSP register pointer
            // fetched from a global) evades it, and timing-sensitive MMIO reads
            // then diverge between the AOT run and the replay (downcount is
            // consumed per-block by AOT but per-instruction by the interpreter).
            // Flag the likely false positive instead of leaving a mystery.
            for (int gi = 0; gi < 32; gi++)
            {
              ...
              if (av >= MMIO_RANGE_START && av < MMIO_RANGE_END && av == interp_result.gpr[gi])
              {
                fmt::print(stderr,
                           ...
                           "likely reads hardware through a runtime-loaded pointer, which "
                           "the MMIO heuristic cannot detect. Probable harness false "
                           "positive, not a codegen bug.\n",
                           gi, av);
                break;
              }
            }
          ...
          if (compare_count % 2000 == 0)
            fmt::print(stderr,
                       "AOT_COMPARE: {} comparisons ({} unique blocks), {} divergences, "
                       "PC={:#010x}\n",
                       compare_count, compared_keys.size(), found_divergence ? 1 : 0,
                       m_ppc_state.pc);
...
void AotHarness::CaptureSnapshot(PPCSnapshot& snap)
{
...
void AotHarness::LogDivergence(u32 block_pc, u32 num_instr, const PPCSnapshot& pre,
...
bool AotHarness::BlockReadsTimebase(u32 block_addr, u32 num_instructions)
{
...
bool AotHarness::BlockAccessesMMIO(const PPCSnapshot& pre, u32 block_addr, u32 num_instructions)
{
...
int AotHarness::RunInterpreterBlock(Interpreter& interp, u32 block_addr, u32 num_instructions,
